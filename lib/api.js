...
API.prototype._report = function() {
  var args = _.map(arguments, function(arg) {
    if (_.isObject(arg) || _.isArray(arg)) return JSON.stringify(arg);
    return arg;
  }).join(' ');
  this.emit('report', args);
};
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  this._report('Seed from random with mnemonic:', this.credentials.getSigningInfo());
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  this._report('Seed from mnemonic:', this.credentials.getSigningInfo());
};
...
API.prototype.import = function(str, opts) {
  ...
  this._report('Import from string:', str);
  this._report('Resulting credentials:', this.credentials.getSigningInfo());
};
...
API.prototype._signTxp = function(txp) {
  ...
  this._report('Signing txp');
  this._report('Credentials:', this.credentials.getSigningInfo());
  this._report('Txp:', txp, 'Using xpriv:', xpriv.toString());
