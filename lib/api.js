...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.network || Constants.LIVENET, words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.getFeeLevels = function(network, cb) {
  ...
  $.checkArgument(network || _.contains([Constants.LIVENET, Constants.TESTNET], network));

  self._doGetRequest('/v1/feelevels/?network=' + (network || Constants.LIVENET), function(err, result) {
    if (err) return cb(err);
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  if (!_.contains([Constants.TESTNET, Constants.LIVENET], network)) return cb(new Error('Invalid network'));
