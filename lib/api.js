...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var info = function() {
    return _.map(txp.inputs, 'path').join(',') + ' Nr of inputs:' + txp.inputs.length + ' Nr of derived privs:' + privs.length + ' Signatures:' + _.map(signatures, function(x) {
  ...
  self.getPayPro(txp, function(err, paypro) {
    ...
    if (_.isEmpty(signatures)) {
      try {
        ...
        log.error('Error signing tx:' + ex + ' Base Derivation Path:' + self.credentials.getBaseAddressDerivationPath());
        return cb(ex);
      }
    }
