...
API.signTxp = function(txp, xpriv) {
  ...
  var info = function() {
    var x  = [];
    x[0] = 'Paths: ' + _.map(txp.inputs, 'path').join(',');
    x[1] = 'Nr of inputs:' + txp.inputs.length;
    x[2] = 'Nr of derived privs:' + privs.length;
    x[3] = 'Signatures:' + _.map(signatures, function(x) {
      return x && x.signature ? x.signature.toDER().toString('hex') : '';
    }).join(',');

// Sensitive data
    // x[4] = 'privs:' + _.map(privs, function(x) {
    //   return x ? x.toString() : '';
    // }).join(', ');
    //
    // x[5] = 'xpriv:' + xpriv.toString();

    return x.join(' | ');
  };


  $.checkState(txp.inputs.length, "Transaction has not inputs");
  ...
  $.checkState(!_.isEmpty(privs), "No private key was derived for inputs paths: " + info());
  ...
  signatures = _.flatten(signatures);
  $.checkState(!_.isEmpty(signatures), "No signatures after getSignatures:" + info());

  signatures = _.map(_.sortBy(signatures, 'inputIndex'), function(s) {
    return s.signature.toDER().toString('hex');
  });
  $.checkState(signatures.length == txp.inputs.length, "No enough signatures after flatten:" + info());

  return signatures;
};

API.prototype._signTxp = function(txp) {
  var xpriv = this.credentials.getDerivedXPrivKey();
  $.checkState(xpriv.hdPublicKey.toString() == this.credentials.xPubKey, 'Failed to derived the correct xPriv:' + xpriv.toString() + ' expected:' + this.credentials.xPrivKey);
  return API.signTxp(txp, xpriv);
};
  ...
  self.getPayPro(txp, function(err, paypro) {
    ...
    if (_.isEmpty(signatures)) {
      try {
        ...
        ex = ex + ' Base Derivation Path:' + self.credentials.getBaseAddressDerivationPath();
