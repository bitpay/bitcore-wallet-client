...
Credentials.packToJson = function(str) {
  str = str.split(SEP).map(function(v) {
...
Credentials.pack = function(values) {
  if (JSON.stringify(values).indexOf(SEP) >= 0)
    throw new Error('Cannot compress credentials containing  ' + SEP);
...
Credentials.prototype.exportCompressed = function() {
  ...
  return Credentials.pack(values);
};

Credentials.importCompressed = function(compressed, password) {
  var list;

  if (compressed.substr(0, 1) != '[') {
    compressed = Credentials.packToJson(compressed);
  }
  ...
  // Remove version
  var version = parseVersion(list[0]);
  ...
  //
  x.m = parseInt(x.m);
  x.xPrivKeyEncrypted = x.xPrivKeyEncrypted || null;
  if (_.isObject(x.xPrivKeyEncrypted))
   x.xPrivKeyEncrypted = JSON.stringify(x.xPrivKeyEncrypted);

  if (version >= 2) {
    if (x.publicKeyRing == '')
      x.publicKeyRing = [];

    x.publicKeyRing = _.map(x.publicKeyRing, function(y) {
      return {
        xPubKey: y[0],
        requestPubKey: y[1],
        isTemporaryRequestKey: y[2] || false,
        copayerName: y[3]
      };
    });
  }
