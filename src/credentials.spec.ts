...
describe('Credentials', () => {
  ...
  describe('#fromObj', () => {
    ...
    it('should throw error importing from incomplete obj', () => {
      const c = new Credentials();
      const data = TestData.credential_btc;
      data['xPrivKey'] = data['xPubKey'] = data[
        'xPrivKeyEncrypted'
      ] = undefined;
      expect(() => {
        c.fromObj(data);
  ...
  describe('#getDerivedXPrivKey', () => {
    ...
    it('should derive extended private key from master testnet', () => {
      ...
      c.fromExtendedPrivateKey(
        'btc',
        'tprv8ZgxMBicQKsPfPX8avSJXY1tZYJJESNg8vR88i8rJFkQJm6HgPPtDEmD36NLVSJWV5ieejVCK62NdggXmfMEHog598PxvXuLEsWgE6tKdwz',
        0,
        'BIP44',
      );
      const xpk = c.getDerivedXPrivKey().toString();
      expect(xpk).to.equal(
        'tprv8gBu8N7JbHZs7MsW4kgE8LAYMhGJES9JP6DHsj2gw9Tc5PrF5Grr9ynAZkH1LyWsxjaAyCuEMFKTKhzdSaykpqzUnmEhpLsxfujWHA66N93',
      );
    });
    ...
    it('should derive extended private key from master livenet (BIP45)', () => {
      ...
      const xpk = c.getDerivedXPrivKey().toString();
      expect(xpk).to.equal(
        'xprv9vDaAbbvT8LHKr8v5A2JeFJrnbQk6ZrMDGWuiv2vZgSyugeV4RE7Z9QjBNYsdafdhwEGb6Y48DRrXFVKvYRAub9ExzcmJHt6Js6ybJCSssm',
      );
    });
  ...
  describe('#fromExtendedPrivateKey', () => {
    it('should create credentials from seed', () => {
      ...
      expect(c.xPrivKey).to.equal(
        'xprv9s21ZrQH143K2TjT3rF4m5AJcMvCetfQbVjFEx1Rped8qzcMJwbqxv21k3ftL69z7n3gqvvHthkdzbW14gxEFDYQdrRQMub3XdkJyt3GGGc',
      );
      expect(c.xPubKey).to.equal(
        'xpub6DUean44k773kxbUq8QpSmAPFaNCpk5AzrxbFRAMsNCZBGD15XQVnRJCgNd8GtJVmDyDZh89NPZz1XPQeX5w6bAdLGfSTUuPDEQwBgKxfh1',
      );
      expect(c.copayerId).to.equal(
        'bad66ef88ad8dec08e36d576c29b4f091d30197f04e166871e64bf969d08a958',
      );
    ...
    it('should create credentials from seed and walletPrivateKey', () => {
      ...
      const wKey =
        'a28840e18650b1de8cb83bcd2213672a728be38a63e70680b0c2be9c452e2d4d';
      const c = new Credentials();
      c.fromExtendedPrivateKey('btc', xPriv, 0, 'BIP44', {
        walletPrivKey:
          'a28840e18650b1de8cb83bcd2213672a728be38a63e70680b0c2be9c452e2d4d',
    ...
    describe('Compliant derivation', () => {
      ...
      it('should create compliant request key', () => {
        ...
        c.fromExtendedPrivateKey('btc', xPriv, 0, 'BIP44');
        expect(c.requestPrivKey).to.equal(
          '559371263eb0b2fd9cd2aa773ca5fea69ed1f9d9bdb8a094db321f02e9d53cec',
        );
      });
      ...
      it('should accept non-compliant derivation as a parameter when importing', () => {
        ...
        expect(c.getDerivedXPrivKey().toString()).to.equal(
          'tprv8gSy16H5hQ1MKNHzZDzsktr4aaGQSHg4XYVEbfsEiGSBcgw4J8dEm8uf19FH4L9h6W47VBKtc3bbYyjb6HAm6QdyRLpB6fsA7bW19RZnby2',
        );
      });
    });
  ...
  describe('#fromMnemonic', () => {
    ...
    it('should create credentials from mnemonic and passphrase', () => {
      ...
      expect(c.xPrivKey).to.equal(
        'xprv9s21ZrQH143K2LkGEPHqW8w5vMJ3giizin94rFpSM5Ys5KhDaP7Hde3rEuzC7VpZDtNX643bJdvhHnkbhKMNmLx3Yi6H8WEsHBBox3qbpqq',
      );
    });
    ...
    it('should create credentials from mnemonic and passphrase for testnet account 2', () => {
      ...
      expect(c.xPrivKey).to.equal(
        'tprv8ZgxMBicQKsPd9yntx9LfnZ5EUiFvEm14L4BigEtq43LrvSJZkT39PRJA69r7sCsbKuJ69fMTzWVkeJLpXhKaQDe5MJanrxvCGwEPnNxN85',
      );
      expect(c.network).to.equal('testnet');
      expect(c.xPubKey).to.equal(
        'tpubDCoAP4Ut9MXK5CakPFPudKAP4yCw6Xr7uzV2129v2LTa3eBoPoUGMqi2y3kmh83oRGX93m7EehB6LWan5GTSVD8yUnV5Jc7Kjzfa3Zsf8nE',
      );
    ...
    it('should create credentials from mnemonic (ES)', () => {
      ...
      c.fromMnemonic('btc', 'livenet', words, '', 0, 'BIP44');
      expect(c.xPrivKey).to.equal(
        'xprv9s21ZrQH143K3H3WtXCn9nHtpi7Fz1ZE9VJErWErhrGL4hV1cApFVo3t4aANoPF7ufcLLWqN168izu3xGQdLaGxXG2qYZF8wWQGNWnuSSon',
      );
      expect(c.network).to.equal('livenet');
    });
    ...
    describe('Compliant derivation', () => {
      it('should create compliant base address derivation key from mnemonic', () => {
        const words =
          'shoulder sphere pull seven top much black copy labor dress depth unit';
        const c = new Credentials();
        c.fromMnemonic('btc', 'livenet', words, '', 0, 'BIP44');
        expect(c.xPrivKey).to.equal(
          'xprv9s21ZrQH143K3WoNK8dVjQJpcXhqfwyuBTpuZdc1ZVa9yWW2i7TmM4TLyfPrSKXctQuLgbg3U1WJmodK9yWM26JWeuh2vhT6bmsPPie688n',
        );
        expect(c.xPubKey).to.equal(
          'xpub6DVMaW3r1CcZcsUazSHspjRfZZJzZG3N7GRL4DciY54Z8M4KmRSDrq2hd75VzxKZDXPu4EKiAwCGwiXMxec2pq6oVgtZYxQHSrgtxksWehx',
        );
      });

      it('should create compliant request key from mnemonic', () => {
        const words =
          'pool stomach bridge series powder mammal betray slogan pass roast neglect reunion';
        const c = new Credentials();
        c.fromMnemonic('btc', 'livenet', words, '', 0, 'BIP44');
        expect(c.xPrivKey).to.equal(
          'xprv9s21ZrQH143K3ZMudFRXpEwftifDuJkjLKnCtk26pXhxQuK8bCnytJuUTGkfvaibnCxPQQ9xToUtDAZkJqjm3W62GBXXr7JwhiAz1XWgTUJ',
        );
        expect(c.requestPrivKey).to.equal(
          '7582efa9b71aefa831823592d753704cba9648b810b14b77ee078dfe8b730157',
        );
      });
      it('should accept non-compliant derivation as a parameter when importing', () => {
        const c = new Credentials();
        c.fromMnemonic(
          'btc',
          'testnet',
          'level unusual burger hole call main basic flee drama diary argue legal',
          '',
          0,
          'BIP44',
          {
  ...
  describe('#createWithMnemonic #fromMnemonic roundtrip', () => {
    _.each(['en', 'es', 'ja', 'zh', 'fr'], lang => {
      it(
        'should verify roundtrip create/from with ' + lang + '/passphrase',
        () => {
          const c = new Credentials();
